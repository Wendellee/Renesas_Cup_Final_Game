...
void encode_thread_create(void);
static void encode_thread_func(void *pvParameters);
void rtos_startup_err_callback(void *p_instance, void *p_data);
void rtos_startup_common_init(void);
extern uint32_t g_fsp_common_thread_count;

const rm_freertos_port_parameters_t encode_thread_parameters = { .p_context =
		(void*) NULL, };
...
void encode_thread_create(void) {
...
#if 1
	encode_thread = xTaskCreateStatic(
#else
	...
	if (NULL == encode_thread) {
		rtos_startup_err_callback(encode_thread, 0);
	}
#else
                    if (pdPASS != encode_thread_create_err)
                    {
                        rtos_startup_err_callback(encode_thread, 0);
                    }
                    #endif
...
static void encode_thread_func(void *pvParameters) {
	...
	encode_thread_entry(pvParameters);
}
