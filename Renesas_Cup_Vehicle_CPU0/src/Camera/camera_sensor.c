...
fsp_err_t camera_write_array (sensor_reg_t const * p_array)
{
    ...
    while (CONFIG_TABLE_END_DETECT != p_array->reg)
    {
        if(p_array->reg != REQUEST_SOFTWARE_WAIT)
        {
            ...
            if (FSP_SUCCESS != err)
            {
                g_printf("[CAM][OV5640][ERR] Write reg 0x%04X = 0x%02X failed: %u\r\n",
                         (unsigned int) p_array->reg,
                         (unsigned int) p_array->val,
                         (unsigned int) err);
                return err;
            }
            ...
            if (FSP_SUCCESS != err)
            {
                g_printf("[CAM][OV5640][ERR] Read reg 0x%04X failed: %u\r\n",
                         (unsigned int) p_array->reg,
                         (unsigned int) err);
                return err;
            }
            ...
            {
                g_printf("[CAM][OV5640][ERR] Verify reg 0x%04X failed, write=0x%02X, read=0x%02X.\r\n",
                         (unsigned int) p_array->reg,
                         (unsigned int) p_array->val,
                         (unsigned int) value);
...
/*
 *[@name] camera_configure_clocks
 *[@type] static function
 *[@usage] 配置OV5640内部PLL与像素时钟
 *[@argument] none
 *[@return] FSP error code
 */
static fsp_err_t camera_configure_clocks(void)
...
/*
 *[@name] camera_set_mipi_virtual_channel
 *[@type] static function
 *[@usage] 配置MIPI CSI虚拟通道
 *[@argument] vchannel
 *[@return] FSP error code
 */
static fsp_err_t camera_set_mipi_virtual_channel(uint32_t vchannel)
