...
void command_rx_thread_create(void);
static void command_rx_thread_func(void *pvParameters);
...
const rm_freertos_port_parameters_t command_rx_thread_parameters = {
		.p_context = (void*) NULL, };
...
#if 1
	command_rx_thread = xTaskCreateStatic(
#else
			...
			6,
#if 1
			(StackType_t*) &command_rx_thread_stack,
			(StaticTask_t*) &command_rx_thread_memory
#else
                        & command_rx_thread
                        #endif
			);
...
#if 1
	if (NULL == command_rx_thread) {
		rtos_startup_err_callback(command_rx_thread, 0);
	}
#else
                    if (pdPASS != command_rx_thread_create_err)
                    {
                        rtos_startup_err_callback(command_rx_thread, 0);
                    }
                    #endif
...
static void command_rx_thread_func(void *pvParameters) {
	...
	command_rx_thread_entry(pvParameters);
}
