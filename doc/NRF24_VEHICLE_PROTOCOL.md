...
| 数据率 | 2 Mbps |
| 地址宽度 | 5 字节 |
| 控制链路地址 | `43 4D 44 52 58`（ASCII `CMDRX`） |
| 视频链路地址 | `56 49 44 45 4F`（ASCII `VIDEO`） |
| RF CRC | 2 字节 |
| Dynamic Payload | 开启 |
| 控制链路 Auto ACK | 开启 |
| 控制链路重发 | ARD=5，即 1500 µs；ARC=15 |
| 视频链路 Auto ACK | 当前实机联调版本关闭；使用边界包 3 次、DATA 2 次的冗余发送 |
| 字节序 | 所有多字节整数均为小端序 |
...
因此小车端应优先保持低延迟和严格顺序，不应为旧帧长时间阻塞新帧。当前实机 ACK 回程持续出现 `MAX_RT=4`，所以视频不再等待 ACK：START/END 各发 3 次、每个 DATA 连续发 2 次、每 4 个分片让出 1 tick。小车端保留的三包 NO_ACK 批发实现由 `VIDEO_FAST_BATCH_NO_ACK_ENABLE` 控制，默认值为 `0`，暂不编译。

## 10. 联调检查表

### 控制链路

- 小车 RX 使用信道 76、2 Mbps、地址 `43:4D:44:52:58`（`CMDRX`）。
...
- 小车 TX 使用信道 100、2 Mbps、地址 `56:49:44:45:4F`（`VIDEO`）。
- 当前实机联调版本关闭视频 Auto ACK，使用 START/END 三发、DATA 双发冗余。
- 每包固定发送 32 字节。
