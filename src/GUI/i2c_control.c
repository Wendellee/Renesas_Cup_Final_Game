...
static fsp_err_t i2c_master_wait_event (const i2c_master_event_t i2c_event)
{
    ...
    uint16_t  time_out = I2C_TIMEOUT_COUNT;
    ...
    while(i2c_event != g_i2c_event_for_peripheral)//its should be RESET_VALUE at the beginning,once the event is received ,it will be updated to the event type in callback function,then the loop will be exited.
    {
        ...
        {
            /* Clear the failed transfer before camera_open retries.  Without
             * this, the next address phase may inherit the previous NACK/bus
             * state and abort immediately as well. */
            (void) R_IIC_MASTER_Abort(&g_i2c_master_for_peripheral_ctrl);
            g_i2c_event_for_peripheral = (i2c_master_event_t) RESET_VALUE;
            APP_ERR_PRINT(" ** Error EVENT_ABORTED received during perform i2c operation **\r\n");
        ...
        else
        {
            ...
            {
                /* Do not leave a timed-out restart/transfer active: it can make
                 * every following GT911 access fail or receive a stale event. */
                (void) R_IIC_MASTER_Abort(&g_i2c_master_for_peripheral_ctrl);
                g_i2c_event_for_peripheral = (i2c_master_event_t) RESET_VALUE;
